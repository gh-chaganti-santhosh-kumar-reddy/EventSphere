    ...
    public static class EventMapper
    {
        public static EventDto ToDto(Event evt)
        {
            return new EventDto
            {
                ...
                Occurrences = evt.Occurrences?.Select(o => new EventOccurrenceDto
                {
                ...
                Speakers = evt.Speakers?.Select(s => new EventSpeakerDto
                {
                    Name = s.Name,
                    Bio = s.Bio,
                    PhotoUrl = s.PhotoUrl
                }).ToList(),
                Faqs = evt.Faqs?.Select(f => new EventFaqDto
                {
                    Question = f.Question,
                    Answer = f.Answer
                }).ToList(),
                Media = evt.Media?.Select(m => new EventMediaDto
                {
